...
export default function StockBalancesView() {
  ...
  if (isError) {
    return (
      <div style={containerStyle}>
        <div role="alert" aria-live="assertive" data-testid="stock-balances-error" style={{ textAlign: 'center', padding: '32px 16px' }}>
  ...
  if (stocks.length === 0) {
    return (
      <div style={containerStyle}>
        <div aria-live="polite" data-testid="stock-balances-empty" style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--color-text-secondary, #6b7280)' }}>
          ...
          <tbody>
            {stocks.map((stock, index) => (
              <tr
                key={stock.ticker}
                style={{
                  borderBottom: index < stocks.length - 1 ? '1px solid var(--color-neutral-100, #f3f4f6)' : 'none',
                }}
              >
                <td style={bodyCellStyle}>
                  <span style={{ fontFamily: 'monospace', fontWeight: 600, color: 'var(--color-text-primary, #111827)' }}>{stock.ticker}</span>
                </td>
                <td style={bodyCellStyle}>
                ...
                <td style={bodyCellStyle}>
                  <span style={{ fontWeight: 600, fontFamily: 'monospace', fontVariantNumeric: 'tabular-nums', color: 'var(--color-text-primary, #111827)' }}>
