          ...
          <tbody>
            {Array.from({ length: 5 }, (_, i) => (
              <tr key={i} style={{ borderBottom: i < 4 ? '1px solid var(--color-neutral-100, #f3f4f6)' : 'none' }}>
                ...
                <td style={bodyCellStyle}>
                  <div style={{ width: '100px', height: '14px', background: 'var(--color-neutral-100, #f3f4f6)', borderRadius: '4px', animation: 'pulse 2s infinite ease-in-out' }} />
                </td>
              </tr>
            ))}
          </tbody>
...
export default function CurrencyAllocationsView() {
  ...
  if (isError) {
    return (
      <div style={containerStyle}>
        <div role="alert" aria-live="assertive" data-testid="currency-allocations-error" style={{ textAlign: 'center', padding: '32px 16px' }}>
  ...
  if (allocations.length === 0) {
    return (
      <div style={containerStyle}>
        <div aria-live="polite" data-testid="currency-allocations-empty" style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--color-text-secondary, #6b7280)' }}>
          ...
          <tbody>
            {allocations.map((allocation, index) => (
              <tr
                ...
                style={{
                  borderBottom: index < allocations.length - 1 ? '1px solid var(--color-neutral-100, #f3f4f6)' : 'none',
                }}
              >
                <td style={bodyCellStyle}>
                  <span style={{ fontWeight: 600, fontFamily: 'monospace', color: 'var(--color-text-primary, #111827)' }}>
                ...
                <td style={bodyCellStyle}>
                  ...
                  <div style={{ marginTop: '4px', height: '4px', borderRadius: '2px', background: 'var(--color-neutral-100, #f3f4f6)', overflow: 'hidden' }}>
                    <div
                      style={{
                        height: '100%',
                ...
                <td style={bodyCellStyle}>
                  <span style={{ fontWeight: 500, fontFamily: 'monospace', fontVariantNumeric: 'tabular-nums', color: 'var(--color-text-primary, #111827)' }}>
