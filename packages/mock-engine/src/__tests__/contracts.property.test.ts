...
import { createGraphQLHandlers } from '../handlers/graphql.js';
import { createRestHandlers } from '../handlers/rest.js';
      ...
      fc.property(
        ...
        (count) => {
          ...
          for (const alloc of allocations) {
            ...
            expect(alloc.allocationPercentage).toBeLessThanOrEqual(100.00);
  ...
  it('allocationPercentage values sum to exactly 100.00', () => {
    fc.assert(
      fc.property(
        ...
        (count) => {
          const allocations = generateCurrencyAllocations(count);
          const sum = allocations.reduce((acc, a) => acc + Math.round(a.allocationPercentage * 100), 0);
...
describe('Property 5: GraphQL Response Schema Conformance', () => {
  ...
  it('stock balance query returns only requested fields', () => {
    fc.assert(
      fc.property(
        subsetArbitrary(allStockFields),
        (requestedFields) => {
  ...
  it('currency allocation query returns only requested fields', () => {
    fc.assert(
      fc.property(
        subsetArbitrary(allCurrencyFields),
        (requestedFields) => {
  ...
  it('transaction query returns only requested fields', () => {
    fc.assert(
      fc.property(
        subsetArbitrary(allTransactionFields),
        (requestedFields) => {
...
describe('Property 7: Error Rate Statistical Conformance', () => {
  it('error proportion is within 3 standard deviations of configured rate', () => {
    fc.assert(
      fc.property(
        fc.double({ min: 0.01, max: 0.30, noNaN: true }),
        (errorRate) => {
